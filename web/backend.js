let ws = new WebSocket("ws://192.168.1.7:9000"); // IP server
...
function StopProc(event) {
    event.preventDefault();

    const procIDInput = document.getElementById('stopproc');
    const processID = procIDInput.value;

    if (ws.readyState === WebSocket.OPEN) {
        ws.send("stop_process:" + processID);
        procIDInput.value = ""
    }
    else {
        log("WebSocket chưa kết nối xong.");
...
let stopProc = document.getElementById("StopProc");
stopProc.addEventListener('submit', StopProc);
